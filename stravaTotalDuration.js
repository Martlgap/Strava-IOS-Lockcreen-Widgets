...
// This is the maximum duration [h] for the gauge. For example put your weekly goal in here
let maxDuration = 10;
...
function circularRange(val, min, max, width, height, desc) {
  ...
  context.drawTextInRect(desc.toString(), new Rect(width / 4.5, (height * 7) / 10, width / 1, (height * 3) / 10));
  return context;
}
...
let image = circularRange(latestActivity.toFixed(0), 0, maxDuration, 175, 175, "h/week").getImage();
